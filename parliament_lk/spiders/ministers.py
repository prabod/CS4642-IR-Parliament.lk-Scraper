...
from parliament_lk.items import Minister
import json

class MinistersSpider(scrapy.Spider):
    name = 'ministers'
    allowed_domains = ['parliament.lk']
    start_urls = ['http://parliament.lk/members-of-parliament/index2.php?option=com_members&task=past&tmpl=component&letter=A&wordfilter=&search_district=&search_legislature=']

    def parse(self,response):
		jsonresponse = json.loads(response.body_as_unicode())
		for r in jsonresponse:
			yield scrapy.Request("http://parliament.lk/en/members-of-parliament/directory-of-past-members/viewMember/%s" %r['mem_intranet_id'], callback=self.parse_minister)
		for i in range(65,91):
			yield scrapy.Request('http://parliament.lk/members-of-parliament/index2.php?option=com_members&task=past&tmpl=component&letter=%s&wordfilter=&search_district=&search_legislature='%chr(i))
    
    def parse_minister(self, response):
		l = ItemLoader(item=Minister(), response=response)
